...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp...')}`);
    ...
    conn.on('open', async () => {
        ...
        if (config.AFPLK == 'SHIBI' || config.AFPLK == 'SHIBI' || config.AFPLK == 'Achumwol' || config.AFPLK == 'pinky') {
        //thanks to afnanplk
        console.log(
            chalk.green.bold('thanks for watching -key cofirmed-')
        );
         }
         else if (config.AFPLK !== 'SHIBI' || config.AFPLK !== 'SHIBI' || config.AFPLK !== 'Achumwol' || config.AFPLK !== 'pinky') {
         console.log(
    ...
    setInterval(async () => { 
        ...
        while (getGMTh == 9 && getGMTm == 01) {
            ...
            return await conn.sendMessage(conn.user.jid, '*[ DAILY ANNOUNCEMENTS BY SHIBI SER ]*\n\n' + announce, MessageType.text);
        }
...
*/
                        ...
                        try {
                            ...
                            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                                ...
                                await conn.sendMessage(conn.user.jid, '*~_________~ 💙 ᴀᴄʜᴜᴍᴡᴏʟ ❤️ ~______~*' +
